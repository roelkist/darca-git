...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-git-venv
    export POETRY_HOME := /tmp/poetry-git-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-git-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/git-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-git-cache
endif
